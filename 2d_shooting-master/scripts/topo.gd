...
func _ready():
	while true:
		...
		$CharacterBody2D/topo.play("entra")
		$CharacterBody2D/topo/shadow.visible = true
		await get_tree().create_timer(0.35).timeout
		$CharacterBody2D/topo.play("default")
		shoot_in_four_directions()  # Llamar a la función para disparar en cuatro direcciones
		await get_tree().create_timer(3).timeout
		$CharacterBody2D/topo.play("sale")
		await get_tree().create_timer(0.35).timeout
		$CharacterBody2D/topo.play("esconde")
		$CharacterBody2D/topo/shadow.visible = false
...
func take_damage():
	health -= Global.daño
	$CharacterBody2D/topo.play("damage")
